...
var InitiativeTrackerView = class extends import_obsidian.ItemView {
  ...
  renderCombatants(container) {
    ...
    sortedCombatants.forEach((combatant) => {
      ...
      const tempFramePercentage = Math.min(100, (combatant.currentHp + combatant.tempHp) / combatant.maxHp * 100);
      hpBar.style.width = `${Math.max(0, Math.min(100, hpPercentage))}%`;
      tempHpFrame.style.width = `${Math.max(0, Math.min(100, tempFramePercentage))}%`;
      tempHpFrame.style.left = "0";
