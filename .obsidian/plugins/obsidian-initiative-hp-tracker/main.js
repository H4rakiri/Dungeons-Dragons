...
var InitiativeTrackerView = class extends import_obsidian.ItemView {
  ...
  renderCombatants(container) {
    ...
    sortedCombatants.forEach((combatant) => {
      ...
      const tempHpFrame = hpBarContainer.createDiv("temp-hp-frame");
      const hpPercentage = combatant.currentHp / combatant.maxHp * 100;
      const tempHpWidth = Math.min(combatant.tempHp, Math.max(0, combatant.maxHp - combatant.currentHp));
      const tempHpPercentage = tempHpWidth / combatant.maxHp * 100;
      hpBar.style.width = `${Math.max(0, Math.min(100, hpPercentage))}%`;
      tempHpFrame.style.width = `${Math.max(0, Math.min(100, tempHpPercentage))}%`;
      tempHpFrame.style.left = `${Math.max(0, Math.min(100, hpPercentage))}%`;
